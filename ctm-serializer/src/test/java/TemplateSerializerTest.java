...
public class TemplateSerializerTest extends TestCase {
	...
	public void testNameTemplates() throws Exception {
		...
		NameEntry n = entryFactory.newNameEntry(new VariableParam("$name"),
				new WildcardParam("?c"));
		n.setScopeEntry(entryFactory.newScopeEntry(new VariableParam("$theme1"),new VariableParam("$theme2"),new VariableParam("$theme3")));
		n.setReifierEntry(entryFactory.newReifierEntry(new VariableParam("$reifier")));		
		t.add(n);
